...
async function MagazineSection({ allPosts }: { allPosts: ArticleData[] }) {
  ...
  return (
    <div className="flex flex-col w-full mb-10 sm:mb-16 pb-8 sm:pb-12 relative">
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-gray-300 to-transparent"></div>
      <Link href={`/article/${magTop.slug}`} className="flex flex-col md:flex-row gap-4 sm:gap-6 mb-8 sm:mb-10 group cursor-pointer">
        <div className="w-full md:w-[340px] lg:w-[380px] shrink-0 aspect-[16/9] rounded-lg overflow-hidden relative border border-gray-100">
        ...
        <div className="flex-1 flex flex-col justify-center">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 leading-snug group-hover:text-[#2563eb] transition-colors mb-2 sm:mb-3">
            {magTop.title}
          </h2>
      ...
      <Link href={`/article/${magHero.slug}`} className="relative w-full aspect-[16/9] md:aspect-[2.5/1] rounded-lg overflow-hidden mb-8 sm:mb-12 group cursor-pointer block">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end items-center pb-6 sm:pb-10 px-4 text-center">
          <span className="bg-[#ef4444] text-white text-[10px] sm:text-[11px] font-bold px-3 py-1 uppercase tracking-widest mb-3 sm:mb-4">{magHero.category}</span>
          <h2 className="text-xl sm:text-2xl md:text-4xl lg:text-5xl font-bold text-white max-w-4xl leading-tight drop-shadow-lg">
            {magHero.title}
          </h2>
        </div>
      </Link>

      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-x-4 sm:gap-x-6 gap-y-6 sm:gap-y-10">
...
async function AsymmetricSection({ allPosts }: { allPosts: ArticleData[] }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 sm:gap-10 mb-10 sm:mb-16">
...
async function NotableSection({ allPosts }: { allPosts: ArticleData[] }) {
  ...
  return (
    <div className="mb-4 bg-white/70 backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-10 border border-gray-200/60 shadow-[0_8px_32px_rgba(0,0,0,0.06)]">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-gray-200/70 pb-4 mb-6 sm:mb-10 gap-3">
        <h2 className="text-lg sm:text-2xl font-bold text-gray-900 uppercase tracking-widest">Đáng chú ý</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
        {notableGrid.map((post, idx) => {
          ...
          return (
            <div key={`notable-${post.id}`} className="flex flex-col h-full bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
...
async function VideoSection() {
  ...
  return (
    <div className="mb-10 sm:mb-16 bg-gray-950 rounded-xl p-4 sm:p-6 md:p-10 text-white shadow-2xl">
      <div className="flex items-center justify-between border-b border-gray-800 pb-4 mb-6 sm:mb-10">
        <h2 className="text-lg sm:text-2xl font-bold uppercase tracking-widest flex items-center gap-2 sm:gap-3">
          <PlayCircle className="text-[#ff0000]" size={22} /> Video Mới
        </h2>
...
async function EditorPicksSection({ allPosts }: { allPosts: ArticleData[] }) {
  ...
  return (
    <div className="mb-4 bg-white/70 backdrop-blur-md rounded-2xl p-4 sm:p-6 md:p-10 border border-gray-200/60 shadow-[0_8px_32px_rgba(0,0,0,0.06)]">
      <h2 className="text-lg sm:text-2xl font-bold text-gray-900 uppercase tracking-widest border-b border-gray-200/70 pb-4 mb-6 sm:mb-10">
        Lựa chọn của Editor
      </h2>
...
export default async function Home() {
  ...
  return (
    <main className="relative min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8 flex flex-col w-full">
      ...
      <div className="w-full flex items-center gap-4 mb-8 sm:mb-12">
