...
export function HeroGridCard({ article }: CardProps) {
  return (
    <Link href={`/article/${article.slug}`} className="relative w-full aspect-[16/9] rounded-lg overflow-hidden group cursor-pointer block">
      <SafeImage src={article.coverImage} alt={article.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="(max-width: 768px) 100vw, 50vw" fallbackSrc={article.contentFirstImage} />
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-between p-4 sm:p-6">
        ...
        <h3 className="text-lg sm:text-xl md:text-2xl font-serif font-bold text-white leading-snug drop-shadow-md line-clamp-3">
          {article.title}
        </h3>
      </div>
    </Link>
...
export function SmallGridCard({ article }: CardProps) {
  return (
    <Link href={`/article/${article.slug}`} className="flex flex-col group cursor-pointer">
      <div className="relative w-full aspect-[16/9] rounded-lg overflow-hidden mb-3 sm:mb-4">
        <SafeImage src={article.coverImage} alt={article.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="(max-width: 768px) 100vw, 33vw" fallbackSrc={article.contentFirstImage} />
        <div className="absolute top-3 left-3">
          <span className="bg-[#ef4444] text-white text-[10px] font-bold px-3 py-1 uppercase tracking-wider">{article.category}</span>
        </div>
      </div>
      <h3 className="text-[14px] sm:text-[16px] font-serif font-bold text-gray-900 leading-snug group-hover:text-[#2563eb] transition-colors mb-1.5 sm:mb-2 line-clamp-2">
        {article.title}
      </h3>
...
export function MagazineGridCard({ article }: CardProps) {
  return (
    <Link href={`/article/${article.slug}`} className="flex flex-col items-center text-center group cursor-pointer">
      <div className="relative w-full aspect-[16/9] rounded-lg mb-4 sm:mb-6">
        <div className="w-full h-full rounded-lg overflow-hidden relative border border-gray-100">
          <SafeImage src={article.coverImage} alt={article.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="(max-width: 640px) 50vw, 25vw" fallbackSrc={article.contentFirstImage} />
        </div>
      ...
      <h3 className="text-[13px] sm:text-[17px] font-bold text-gray-900 leading-snug group-hover:text-[#2563eb] transition-colors mb-2 sm:mb-3 line-clamp-3 px-1 sm:px-2">
        {article.title}
      </h3>
      <div className="flex items-center justify-center text-[10px] sm:text-[12px] text-gray-400 gap-3 mt-auto">
  ...
  return (
    <Link href={`/article/${article.slug}`} className="flex flex-col sm:flex-row gap-6 border-b border-gray-100 pb-8 last:border-0 group cursor-pointer">
      <div className="w-full sm:w-[280px] shrink-0 aspect-[16/9] rounded-lg overflow-hidden relative">
        <SafeImage src={article.coverImage} alt={article.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="280px" fallbackSrc={article.contentFirstImage} />
      </div>
  ...
  return (
    <div className="group cursor-pointer flex flex-col h-full">
      <Link href={`/article/${article.slug}`} className="flex flex-col h-full">
        <div className="relative overflow-hidden rounded-lg mb-3 sm:mb-4 aspect-[16/9] shrink-0">
          <SafeImage src={article.coverImage} alt={article.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" fallbackSrc={article.contentFirstImage} />
        </div>
        ...
        <h3 className="text-lg sm:text-xl font-bold leading-snug mb-2 sm:mb-3 group-hover:text-[#2563eb] transition-colors text-gray-900 line-clamp-2">{article.title}</h3>
        <p className="text-gray-600 text-[13px] sm:text-[14px] leading-relaxed line-clamp-2 flex-grow">{stripMarkdown(article.excerpt)}</p>
  ...
  return (
    <Link href={`/article/${article.slug}`} className="group cursor-pointer flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-all duration-300">
      <div className="w-full aspect-[16/9] overflow-hidden relative">
        <SafeImage src={article.coverImage} alt={article.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 1024px) 100vw, 50vw" fallbackSrc={article.contentFirstImage} />
      </div>
      <div className="p-4 sm:p-6 flex flex-col flex-grow">
        <span className="text-[#2563eb] text-[10px] sm:text-[11px] font-bold uppercase tracking-widest mb-2">{article.category}</span>
        <h3 className="text-xl sm:text-2xl font-bold text-gray-900 leading-snug mb-2 sm:mb-3 group-hover:text-[#2563eb] transition-colors line-clamp-2">
          {article.title}
        </h3>
        <p className="text-gray-600 text-[14px] sm:text-[15px] leading-relaxed line-clamp-3 mb-4 sm:mb-6 flex-grow">
          {stripMarkdown(article.excerpt)}
        </p>
        <div className="flex items-center justify-between text-[10px] sm:text-[12px] text-gray-400 mt-auto pt-3 sm:pt-4 border-t border-gray-100">
          <span>By <span className="font-bold text-gray-700">{article.author.toUpperCase()}</span></span>
          <span className="flex items-center gap-1"><Clock size={12} /> {formatDate(article.date)}</span>
        </div>
      </div>
  ...
  return (
    <Link href={`/article/${article.slug}`} className="group cursor-pointer flex flex-col bg-gray-900 p-2 rounded-lg hover:bg-gray-800 transition-colors">
      <div className="relative overflow-hidden rounded-md mb-3 aspect-[16/9]">
        <SafeImage src={article.coverImage} alt={article.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105 opacity-80 group-hover:opacity-100" sizes="25vw" fallbackSrc={article.contentFirstImage} />
  ...
  return (
    <a href={video.url} target="_blank" rel="noopener noreferrer" className="group cursor-pointer flex flex-col bg-gray-900 p-2 rounded-lg hover:bg-gray-800 transition-colors h-full">
      <div className="relative overflow-hidden rounded-md mb-3 aspect-[16/9] shrink-0">
        <SafeImage src={video.thumbnail} alt={video.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105 opacity-80 group-hover:opacity-100" sizes="(max-width: 768px) 100vw, 25vw" />
        <div className="absolute inset-0 flex items-center justify-center">
