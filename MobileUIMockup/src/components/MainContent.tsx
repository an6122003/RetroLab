import { Clock, ShieldCheck, MonitorPlay, BrainCircuit, PlayCircle, ArrowRight, MessageSquare, RefreshCw, ChevronUp } from 'lucide-react';
...
export default function MainContent() {
  ...
  useEffect(() => {
    ...
    // Simulate story loading
    const timer = setTimeout(() => setIsLoadingStories(false), 1500);
  ...
  return (
    <div className="flex flex-col w-full relative">
      ...
      {/* MOBILE ONLY: Pull to Refresh Visual */}
      <div className="md:hidden flex justify-center py-2 -mt-8 mb-4 opacity-30">
        <motion.div
          animate={{ rotate: 360 }}
      ...
      {/* MOBILE ONLY: Stories/Highlights Section */}
      <div className="md:hidden flex overflow-x-auto gap-4 pb-6 mb-8 no-scrollbar -mx-4 px-4">
        {isLoadingStories ? (
          // Skeleton Loaders
          [...Array(6)].map((_, i) => (
          ...
          <>
            <StoryItem 
              label="S26 Ultra" 
              image="https://picsum.photos/seed/s26story/150/150" 
              active 
            />
            <StoryItem 
              label="iOS 18" 
              image="https://picsum.photos/seed/iosstory/150/150" 
            />
            <StoryItem 
              label="MacBook" 
              image="https://picsum.photos/seed/macstory/150/150" 
            />
            <StoryItem 
              label="AI News" 
              image="https://picsum.photos/seed/aistory/150/150" 
            />
            <StoryItem 
              label="Gaming" 
              image="https://picsum.photos/seed/gamestory/150/150" 
            />
            <StoryItem 
              label="Tips" 
              image="https://picsum.photos/seed/tipsstory/150/150" 
            />
            <StoryItem 
              label="Deals" 
              image="https://picsum.photos/seed/dealstory/150/150" 
            />
          </>
        )}
      </div>
      ...
      <div className="flex flex-col w-full mb-16 border-b border-gray-200 pb-12">
        {/* Top Single Post */}
        <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-10 group cursor-pointer">
          <div className="w-full md:w-[380px] shrink-0 aspect-[16/9] rounded-lg overflow-hidden shadow-sm">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div className="mb-3">
              <span className="text-[#ef4444] text-[10px] md:text-[11px] font-bold uppercase tracking-widest border-b-2 border-[#ef4444] pb-1">Xem xong mua</span>
            </div>
            <h2 className="text-xl md:text-3xl font-bold text-gray-900 leading-snug group-hover:text-[#2563eb] transition-colors mb-3">
            ...
            <div className="flex items-center text-[10px] md:text-[12px] text-gray-400 gap-3">
        ...
        {/* Middle Hero Post */}
        <div className="relative w-full aspect-[16/9] md:aspect-[2.5/1] rounded-lg overflow-hidden mb-12 group cursor-pointer shadow-md">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end items-center pb-6 md:pb-10 px-4 text-center">
            <span className="bg-[#ef4444] text-white text-[9px] md:text-[11px] font-bold px-2 md:px-3 py-1 uppercase tracking-widest mb-2 md:mb-4">Góc nhìn</span>
            <h2 className="text-xl md:text-4xl lg:text-5xl font-bold text-white max-w-4xl leading-tight drop-shadow-lg">
      ...
      <AnimatePresence>
        {showBackToTop && (
          <motion.button
            ...
            className="md:hidden fixed bottom-24 right-6 w-12 h-12 bg-white border border-gray-200 rounded-full shadow-lg flex items-center justify-center text-gray-600 z-40"
          >
            <ChevronUp size={24} />
          </motion.button>
        )}
...
function StoryItem({ label, image, active }: { label: string; image: string; active?: boolean }) {
  return (
    <div className="flex flex-col items-center gap-2 shrink-0 cursor-pointer group">
      <div className={`w-16 h-16 rounded-full p-[2px] ${active ? 'bg-gradient-to-tr from-[#facc15] to-[#2563eb]' : 'bg-gray-200'}`}>
        <div className="w-full h-full rounded-full border-2 border-white overflow-hidden">
          <img src={image} alt={label} className="w-full h-full object-cover group-active:scale-95 transition-transform" />
        </div>
      </div>
      <span className={`text-[10px] font-bold uppercase tracking-tighter ${active ? 'text-[#2563eb]' : 'text-gray-500'}`}>{label}</span>
    </div>
  );
}
